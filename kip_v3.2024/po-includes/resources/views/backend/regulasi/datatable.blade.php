@extends('layouts.app')
@section('title', __('regulasi.datatable_title'))
...
@section('content')
        ...
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb breadcrumb-style1 mg-b-10">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ __('regulasi.datatable_list') }}</li>
                </ol>
            </nav>
            <h4 class="mg-b-0 tx-spacing--1">{{ __('regulasi.datatable_list') }}</h4>
        </div>
        
        <div>
            <a href="{{ url('dashboard/regulasi/create') }}" class="btn btn-sm pd-x-15 btn-white btn-uppercase mg-t-10">
                <i data-feather="plus" class="wd-10 mg-r-5"></i> {{ __('general.add') }}
            </a>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-body">
            {!! Form::open(['url' => 'dashboard/regulasi/deleteall', 'method' => 'post', 'class' => 'form-horizontal']) !!}
                <input type="hidden" name="totaldata" id="totaldata" value="0" />
				<div class="table-responsive" style="overflow-x: auto;">
					<table class="table table-striped" id="regulasis-table">
						<thead>
							<tr>
								<th style="text-align:center;" width="15"></th>
								<th style="text-align:center;" width="25">{{ __('general.id') }}</th>
								<th>{{ __('regulasi.tentang') }}</th>
								<th>{{ __('regulasi.judul') }}</th>
								<th>{{ __('regulasi.keterangan') }}</th>
								<th>{{ __('regulasi.file') }}</th>
								<th>{{ __('regulasi.diunduh') }}</th>
...
$(document).ready(function() {
    $('#regulasis-table').DataTable({
        processing: true,
        serverSide: true,
        stateSave: true,
        ajax: '{{ url("dashboard/regulasi/data") }}',
        autoWidth: false,
        order: [[1, 'desc']],
		columnDefs: [{
				targets: 'no-sort',
        ...
        columns: [
            ...
            { data: 'id', name: 'regulasis.id' },
            { data: 'tentang', name: 'regulasis.tentang' },
            { data: 'judul', name: 'regulasis.judul' },
            { data: 'keterangan', name: 'regulasis.keterangan' },
            { data: 'file', name: 'regulasis.file' },
            { data: 'diunduh', name: 'regulasis.diunduh' },
