@extends('layouts.app')
@section('title', __('permohonaninformasi.edit_title'))
...
@section('content')
	<div class="d-sm-flex align-items-center justify-content-between mg-b-20 mg-lg-b-25 mg-xl-b-20">
		<div>
			...
			<h4 class="mg-b-0 tx-spacing--1">{{ __('permohonaninformasi.edit_title') }}</h4>
		</div>
	...
	<div class="card">
		{!! Form::model($permohonaninformasi, [
			'method' => 'PATCH',
			'url' => ['/dashboard/ppid/permohonaninformasi', Hashids::encode($permohonaninformasi->id)],
			...
			<div class="card-body pd-b-0">
				...
				@include ('backend.ppid.permohonaninformasi.form', ['formMode' => 'edit'])

				@if($permohonaninformasi->file_ktp)
					<div class="form-group">
						<label for="file">{{ __('File yang Sudah Diunggah') }}</label>
						<div>
							<a href="{{ url('po-content/uploads/permohonaninformasi/' . $permohonaninformasi->file_ktp) }}" target="_blank">{{ $permohonaninformasi->file_ktp }}</a>
						</div>
					</div>
				@endif
			</div>
		...
		<div class="pos-absolute t-10 r-10">
			<div class="toast" role="alert" aria-live="assertive" aria-atomic="true" style="opacity:1;">
				<div class="toast-header">
					<h6 class="tx-inverse tx-14 mg-b-0 mg-r-auto">{{ __('permohonaninformasi.notification') }}</h6>
					<button type="button" class="ml-2 mb-1 close tx-normal" data-dismiss="toast" aria-label="Close">
						<span aria-hidden="true">&times;</span>
					</button>
				</div>
				<div class="toast-body bg-gray-100">{{ __('permohonaninformasi.error_gallery') }}</div>
			</div>
		</div>
	</div>
@endsection
